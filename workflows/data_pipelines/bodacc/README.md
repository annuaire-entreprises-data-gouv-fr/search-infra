...
En appliquant des filtres en amont pour ne télécharger que les annonces de la famille
souhaitée, et un `select` pour ne demander que les colonnes utilisées par le
preprocessing.

### Traitement incrémental des annonces

L'historique complet représente plus de 42 millions d'annonces donc le télécharger à chaque fois prenait entre 1h et 1h40.
Seules les annonces des derniers mois glissants sont téléchargées et fusionnées avec le fichier `annonces_couples.csv` récupéré depuis l'object storage.
L'historique complet n'est retéléchargé que si le DAG est lancé avec le paramètre `force_rebuild`,
ou si `annonces_couples.csv` est absent de l'object storage. Il y a un risque de timeout.
...
- **Annonces** :
  ...
  - Traitement incrémental (voir ci-dessus)
