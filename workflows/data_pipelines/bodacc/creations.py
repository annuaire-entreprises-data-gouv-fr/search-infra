...
def _parse_creation_date(acte_str: str) -> str:
    """Extrait la date de création depuis la colonne acte en format json.

    On privilégie la date d'immatriculation au RCS, avec repli sur la date de
    commencement d'activité quand celle ci est absente.
    """
...
def _process_creation_chunk(chunk: pd.DataFrame) -> pd.DataFrame:
    """Traiter un chunk des données de créations du BODACC."""
...
def process_creations(raw_file_path: str, chunk_size: int) -> pd.DataFrame:
    logger.info("Processing creations...")
    chunks_processed = []
    ...
    if not chunks_processed:
        logger.warning("No creations found")
        return pd.DataFrame(columns=_OUTPUT_COLUMNS)

    # Concaténer tous les chunks. On conserve toutes les créations, pas de
