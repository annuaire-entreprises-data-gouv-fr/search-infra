...
def send_notification_success_preprocessing_tchap(**kwargs):
    send_message(
        "\U0001F7E2 Données :" "\nDAG de preprocessing a été exécuté avec succès."
    )


def send_notification_success_indexation_tchap(**kwargs):
...
def send_notification_failure_preprocessing_tchap(context):
    send_message("\U0001F534 Données :" "\nFail DAG de preprocessing sirene!!!!")


def send_notification_failure_indexation_tchap(context):
