...
with DAG(
    ...
    preprocess_rge_data = PythonOperator(
        task_id="preprocess_rge_data",
        python_callable=preprocess_rge_data,
    ...
    compare_versions_file = ShortCircuitOperator(
        ...
        op_args=(
            f"{TMP_FOLDER}{DAG_FOLDER}{DAG_NAME}/data/rge-latest.csv",
            f"{TMP_FOLDER}{DAG_FOLDER}{DAG_NAME}/data/rge-new.csv",
        ),
    )
    ...
    publish_mattermost = PythonOperator(
        ...
        op_args=("Infos Annuaire : Données RGE mises à jour sur l'API !",),
    )

    clean_previous_folder.set_upstream(get_colors)
    preprocess_rge_data.set_upstream(clean_previous_folder)
    get_latest_rge_data.set_upstream(preprocess_rge_data)
    compare_versions_file.set_upstream(get_latest_rge_data)
