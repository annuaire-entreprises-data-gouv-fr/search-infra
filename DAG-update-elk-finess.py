...
from dag_datalake_sirene.data_aggregation.finess import (
...
with DAG(
    ...
    compare_versions_file = ShortCircuitOperator(
        ...
        op_args=(
            f"{TMP_FOLDER}{DAG_FOLDER}{DAG_NAME}/data/finess-latest.csv",
            f"{TMP_FOLDER}{DAG_FOLDER}{DAG_NAME}/data/finess-new.csv",
        ),
    )
    ...
    put_file_latest_to_minio = PythonOperator(
        ...
        op_args=(
            "finess-new.csv",
            f"ae/data_aggregation/{ENV}/finess/finess-latest.csv",
            f"{TMP_FOLDER}{DAG_FOLDER}{DAG_NAME}/data/",
        ),
    )
    ...
    publish_mattermost = PythonOperator(
        ...
        op_args=(
            "Infos Annuaire : Données des FINESS (Santé) mises à jour sur l'API !",
        ),
    )

    clean_previous_folder.set_upstream(get_colors)
    preprocess_finess_data.set_upstream(clean_previous_folder)
    get_latest_finess_data.set_upstream(preprocess_finess_data)
    compare_versions_file.set_upstream(get_latest_finess_data)
